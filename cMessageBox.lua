...
local kMsgBoxScaleX = display.contentWidth/800
local kMsgBoxScaleY = display.contentHeight/600
...
local kMsgBoxDefault_TitleTextFont = "Grinched"--native.systemFontBold
local kMsgBoxDefault_MessageTextFont = "Grinched"--native.systemFont
...
local function setAnchors(obj, messageJustify)
...
captionLabel.anchorY=0;
captionLabel.anchorX=0.5;
--captionLabel:set ReferencePoint(display.TopCenterReferencePoint)
captionLabel.x = dlgX+2 - math.floor(width*0.5) -- edited
captionLabel.y = dlgY + borderSizeY - math.floor(height/2.0)+12 -- edited
...
dlgLeft = dlgX - math.floor(width/2)*2
...
for i=1,count do
--messageLabels[ i ]:set  ReferencePoint(newJustify)
messageLabels[ i ].x = xPos
messageLabels[ i ].y = yPos
setAnchors(messageLabels[ i ], messageJustify);
        ...
        if ( nWhichButton > 0 and nWhichButton <= #buttonReturnValues ) then
            ...
            if ( bMakeTextButton == true ) then
                button = newTextButton{
                    defaultX = math.floor( dlgWidth*0.2 ),
                    defaultY = math.floor( buttonTextSize*1.6-20 ),
                    onPress = dlgOnPress,
...
okayBtn.anchorY=1;
--okayBtn:set ReferencePoint( display.BottomCenterReferencePoint )
...
okayBtn.anchorX=0;
okayBtn.anchorY=1;
--okayBtn:set ReferencePoint( display.BottomLeftReferencePoint )
...
cancelBtn.anchorX=1;
cancelBtn.anchorY=1;
--cancelBtn:set  ReferencePoint( display.BottomRightReferencePoint )
...
yesBtn.anchorX=0;
yesBtn.anchorY=1;
--yesBtn:set ReferencePoint( display.BottomLeftReferencePoint )
yesBtn.x = dlgX - math.floor(width*0.5) + 3*borderSizeX+10
yesBtn.y = dlgY + math.floor(height*0.5) - borderSizeY-30

dialog:insert( noBtn )
noBtn.anchorX=1;
noBtn.anchorY=1;
--noBtn:set ReferencePoint( display.BottomRightReferencePoint )
noBtn.x = dlgX + math.floor(width/2.0) - 3*borderSizeX
noBtn.y = dlgY + math.floor(height/2.0) - borderSizeY-30
...
noBtn.anchorY=1;
--noBtn:set ReferencePoint( display.BottomCenterReferencePoint )
noBtn.x = dlgX
